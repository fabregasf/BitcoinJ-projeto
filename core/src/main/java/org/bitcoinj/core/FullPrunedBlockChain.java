    ...
    protected TransactionOutputChanges connectTransactions(int height, Block block)
        ...
        try {
            ...
            for (final Transaction tx : block.transactions) {
                ...
                // All values were already checked for being non-negative (as it is verified in TransactionTest.verify())
                ...
                if (valueOut.signum() < 0 || valueOut.compareTo(params.getMaxMoney()) > 0)
                    throw new VerificationException("TransactionTest output value out of range");
                if (isCoinBase) {
                    coinbaseValue = valueOut;
                } else {
                    if (valueIn.compareTo(valueOut) < 0 || valueIn.compareTo(params.getMaxMoney()) > 0)
                        throw new VerificationException("TransactionTest input value out of range");
            ...
            if (totalFees.compareTo(params.getMaxMoney()) > 0 || block.getBlockInflation(height).add(totalFees).compareTo(coinbaseValue) < 0)
                throw new VerificationException("TransactionTest fees out of range");
